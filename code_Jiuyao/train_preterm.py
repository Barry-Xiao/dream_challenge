...
meta_data = meta_data[meta_data['collect_period']<=5]
...
# Filtered out observations with "collect_wk<=32" == "collect_period<=5" 
taxonomy_data = taxonomy_data[taxonomy_data['collect_period']<=5]
# Average within each collection period
...
# Filtered out observations with "collect_wk<=32" == "collect_period<=5" 
phylotype_data = phylotype_data[phylotype_data['collect_period']<=5]
# Average within each collection period
...
mydata_input = Data_Reshaper_Input(mydata_input, seq_length=5)
...
learn.save_all(path='D:/DREAM/models_preterm', dls_fname='dls_preterm', model_fname='model_preterm', learner_fname='learner_preterm')
learn = load_learner('D:/DREAM/submissionJiuyao/preterm/learner_preterm.pkl')
...
import pickle
pickle.dump(results, open('preterm_15.pkl', 'wb'))

results_new = pickle.load(open('preterm_15.pkl', 'rb'))
