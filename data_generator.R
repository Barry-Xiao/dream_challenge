...
##outcome either was_preterm or was_early_preterm
outcome = args[1]
...
filter_columns = read.csv(filter_path, header = T)[,1]

data_filtered = read.csv(data_path, header = T) %>% select(c('specimen',filter_columns))
...
write.csv(combo_data_clean , outfile_path, row.names = F, na = "")
